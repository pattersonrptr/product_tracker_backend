...
async def test_get_current_user_success(mock_user_repo_cls, mock_jwt_decode, credentials, db):
...
@patch("src.app.security.auth.jwt.decode")
@patch("src.app.security.auth.UserRepository")
async def test_get_current_user_user_not_found(mock_user_repo_cls, mock_jwt_decode, credentials, db):
    mock_jwt_decode.return_value = {"sub": "john"}
    mock_user_repo = MagicMock()
