...
def test_scrape_data_success(mock_retry, scraper):
    ...
    with patch("src.product_scrapers.scrapers.olx.BeautifulSoup") as bs_mock:
        ...
        soup_mock.find.return_value.__getitem__.side_effect = lambda k: '{"ad": {"subject": "t", "body": "d", "listId": "1", "priceValue": "R$ 100,00", "images": [{"original": "img"}], "user": {"name": "seller"}, "location": {"municipality": "city", "uf": "state"}}}' if k == "data-json" else None
