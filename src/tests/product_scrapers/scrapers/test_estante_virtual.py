...
def test_search_success(mock_retry, scraper):
    ...
    with patch.object(scraper, "_get_products_list", wraps=scraper._get_products_list) as mock_get_products_list:
...
def test_scrape_data_success(mock_extract_initial_state, mock_parse_html, mock_retry, scraper):
    mock_resp = MagicMock()
...
@patch.object(EstanteVirtualScraper, "_parse_html")
@patch.object(EstanteVirtualScraper, "_extract_initial_state")
def test_scrape_data_no_data(mock_extract_initial_state, mock_parse_html, mock_retry, scraper):
    mock_resp = MagicMock()
    mock_resp.content = b"<html></html>"
    mock_retry.return_value = mock_resp
    mock_parse_html.return_value = MagicMock()
